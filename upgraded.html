...
/* SHOOTING STARS - IMPROVED: SIMPLER AND MORE ELEGANT */
...
.shooting-star-trail {
  ...
  filter: blur(0.4px);
  transform-origin: left center;  /* Changed from left to right */
  transform: rotate(225deg);       /* Changed from 45deg to 225deg (45 + 180) */
}


/* Shooting star animation - SIMPLE AND ELEGANT */
...
/* Second type of shooting star animation */
  ...
  <div class="main-content">
    ...
    <div class="container">
      ...
      <div class="flowers">
        ...
        <div class="flower-wrap size-medium" style="top:30px; left:40.5px;">

          <div class="flower"></div>

        </div>

        <div class="flower-wrap size-large" style="top:45.5px; left:125px;">

          <div class="flower"></div>

        </div>

        <div class="flower-wrap size-small" style="top:33.3px; left:160.5px;">

          <div class="flower"></div>

        </div>

        <div class="flower-wrap size-medium" style="top:31.5px; left:187.5px;">
...
// CREATE SHOOTING STAR - SIMPLE AND ELEGANT
...
function createShootingStar() {
  ...
  // START POSITION: From top-left area
  ...
  // Start from top-left, but spread out more
...
const useSecondAnimation = Math.random() > 0.5;
  ...
  if (useSecondAnimation) {
    ...
    duration = 4000 + Math.random() * 600;

  } else {

    animationName = 'shootStar';

    duration = 5500 + Math.random() * 500;
...
function startShootingStars() {
  ...
  const maxStars = 10 + Math.floor(Math.random() * 3); // 4-6 stars (fewer is better)
  ...
  function createStar() {
    ...
    if (starsCreated < maxStars) {
      ...
      const nextDelay = 500 + Math.random() * 1000; // More space between stars

      setTimeout(createStar, nextDelay);

    } else {

      // Longer pause before next shower
...
// CREATE SLOW HALO FOR NAME PROMPT
 ...
 function createNamePromptHalo() {
   ...
   const centerX = rect.left + rect.width / 2.05;
...
function createNamePromptHalo() {
  ...
  // initial + after fonts (fonts can shift layout)
  updateCenter();
  if (document.fonts && document.fonts.ready) {
    document.fonts.ready.then(updateCenter).catch(() => {});
  }

  // update on resize (debounced) + orientation change
...
// 5-SECOND TRANSITION HALO WITH WELCOME NOTE
...
function startTransitionHaloWithWelcome() {
  ...
  const centerX = rect.left + rect.width / 2.075;

  const centerY = rect.top + rect.height / 1.95;
...
// FLOWER ANIMATION - FROM OLD VERSION
